...
- 一度解析したリポジトリは索引に残り、以後は定期実行で自動更新される
- 解析は GitHub Actions 上の Python で走り、リポジトリごとに
  `docs/data/<owner>-<name>.json` と索引 `docs/data/index.json` を出力する
...
**結果は蓄積される。** 一度解析したリポジトリは `docs/data/index.json` に
登録され、以後は 6 時間ごとの定期実行でまとめて更新される。対象を
リポジトリ内に持たないので、追加も削除も Actions の実行だけで完結する。

対象から外したいときは `docs/data/` から該当ファイルを消し、
...
`publish` を有効にして解析してよいのは公開リポジトリだけ。**
private リポジトリを解析するときは `publish` を外して artifact で受け取る
（またはこのリポジトリ自体を private にする）。
